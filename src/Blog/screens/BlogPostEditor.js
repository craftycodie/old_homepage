...
class BlogPostEditor extends React.Component {
  constructor (props) {
    ...
    var postID = null
    if (this.props.match.params.postID) { postID = this.props.match.params.postID }
  ...
  handleChange (event) {
    ...
    if (event.target.id === 'stickyPost' || event.target.id === 'draftPost') { obj[event.target.id] = event.target.checked }
    this.setState(obj)
  }
