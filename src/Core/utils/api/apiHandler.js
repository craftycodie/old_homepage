...
export default class ApiHandler {
  ...
  checkToken () {
    fetch(this.apiConfig.protectedRoute,
      ...
      .then(response => {
        if (!response.ok) { throw Error('Error checking token posts.') } else return response
      })
      .then(r => r.json())
      .then(j => {
  ...
  deletePost (postID, successCallback) {
    ...
    request
      ...
      .end((err, res) => {
        ...
        } else { alert('Failed to delete post: ' + JSON.parse(res.text).message) }
      })
  }
  ...
  editPost (postID, newTitle, newBody, newSticky, newDraft, successCallback) {
    ...
    request
      ...
      .end((err, res) => {
        var success = JSON.parse(res.text).success
        if (!success) { alert('Failed to edit post: ' + JSON.parse(res.text).message) } else if (successCallback) { successCallback(postID) }
      })
  }
  ...
  newPost (newTitle, newBody, newSticky, newDraft, successCallback) {
    ...
    request
      ...
      .end((err, res) => {
        var success = JSON.parse(res.text).success
        if (!success) { alert('Failed to create post: ' + JSON.parse(res.text).message) } else if (successCallback) { successCallback(JSON.parse(res.text).data) }
      })
  }
  ...
  recentPosts (at, count, success, error) {
        ...
        }),
    {
      ...
      .then(response => {
        if (!response.ok) { throw Error('Error retrieving recent posts.') } else return response
      })
      .then(r => r.json())
      .then(j => {
        if (!j.success) { throw Error('Error retrieving recent posts.') }
  ...
  blogPost (postID, success, error) {
    fetch(this.apiConfig.postRoute + '/' + postID,
      ...
      .then(response => {
        if (!response.ok) { throw Error('Error retrieving blog post.') } else return response
      })
      .then(r => r.json())
      .then(j => {
        if (!j.success) { throw Error('Error blog recent post.') }
  ...
  stickyPosts (success, error) {
    fetch(this.apiConfig.stickyPostsRoute,
      ...
      .then(response => {
        if (!response.ok) { throw Error('Error retrieving sticky posts.') } else return response
      })
      .then(r => r.json())
      .then(j => {
        if (!j.success) { throw Error('Error retrieving sticky posts.') }
  ...
  draftPosts (success, error) {
    ...
    fetch(this.apiConfig.draftPostsRoute,
      ...
      .then(response => {
        if (!response.ok) { throw Error('Error retrieving draft posts.') } else return response
      })
      .then(r => r.json())
      .then(j => {
        if (!j.success) { throw Error('Error retrieving draft posts.') }
